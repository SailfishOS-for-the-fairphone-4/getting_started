...
2) Sourcing and Building relevant bits of your chosen Android base (LineageOS 18.1)
3) Packaging/building SailfishOS (SailfishOS 4.5.0.18)
4) Flashing SailfishOS
...
(On most linux distro's these packages are automatically provided)
...
variables are written to some files. In our case (Fairphone 4):

```
HOST $
```

Here we create a file in the $HOME directory called .hadk.env and the write text until EOF (End Of File) is written.
In this case $HOME would be the home folder for the signed in user

```
cat << 'EOF' > $HOME/.hadk.env
```

Then we will write the variables we will use.
ANDROID_ROOT is being used for: #TODO Figure out what it is being used for
VENDOR is being used for: #TODO Figure out what it is being used for
DEVICE is being used for: #TODO Figure out what it is being used for
PORT_ARCH is being used for: #TODO Figure out what it is being used for
...
Then we will write a EOF command so the file will be saved.

```
EOF
```

When this is done the file in your $HOME directory called .hadk.env should contained the following:
...
Then we will do the same just with another file and other content.
